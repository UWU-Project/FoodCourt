...
if(isset($_REQUEST['action']) && !empty($_REQUEST['action'])){
    if($_REQUEST['action'] == 'addToCart' && !empty($_REQUEST['id'])){
        ...
        // Get product details
        $query = $conn->query("SELECT * FROM food_details  WHERE food_id = ".$productID);
        ...
        if(empty($errorMsg)){
            ...
            $insertCust = $conn->query("INSERT INTO customers (firstname, lastname, email, phone, address) VALUES ('".$first_name."', '".$last_name."', '".$email."', '".$phone."', '".$address."')");
            ...
            if($insertCust){
                ...
                if($insertOrder){
                    ...
                    foreach($cartItems as $item){
                        $sql .= "INSERT INTO cart_details (cart_id, member_id, quantity_id) VALUES ('".$orderID."', '".$item['id']."', '".$item['qty']."');";
                    }
