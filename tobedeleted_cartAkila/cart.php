...
<div style="border:#bd6f2f solid 1px;padding:4px 6px 2px 6px" class="card-body">
    ...
    <table width="910" height="auto" style="text-align:center;" class="table table-borderless table-hover" w-auto>
        ...
        foreach ($result as $row) {
            ...
            echo '<td><input class="form-control" type="number" value="' .$row['quantity_value'].'" onchange="updateQuantity(this,' .$row['cart_id']. ')"/></td>';
            echo "<td>" . $symbol['currency_symbol'] . "" . $row['total'] . "</td>";
            echo '<td class="text-right">'. $symbol['currency_symbol'] . "" . $row['total'] .'</td>';
...
<script>
    function updateCartItem(obj,id){
        $.get("cartAction.php", {action:"updateCartItem", id:cart_id, qty:quantity_id.value}, function(data){
            if(data == 'ok'){
                location.reload();
            }else{
